...
public class IRETrackBlueprint extends CustomItem {

    public IRETrackBlueprint(String modID, String name) {
        super(modID, name);
        Fuzzy steel = Fuzzy.STEEL_INGOT;
        IRFuzzy.registerSteelRecipe(this,
                3,
    ...
    public ClickResult onClickBlock(Player player, World world, Vec3i pos, Player.Hand hand, Facing facing, Vec3d hit) {
        ...
        if (world.isServer && hand == Player.Hand.SECONDARY) {
            ...
            if (player.isCrouching()) {
                stackInfo = stackInfo.with((b) -> {
                    b.railBedFill = blockInfo;
                });
            } else {
                stackInfo = stackInfo.with((b) -> {
                    b.railBed = blockInfo;
            ...
            pos = pos.up();
            if (BlockUtil.canBeReplaced(world, pos.down(), true) && (!BlockUtil.isIRRail(world, pos.down()) || (double) world.getBlockEntity(pos.down(), TileRailBase.class).getRailHeight() < 0.5)) {
                pos = pos.down();
            }
            ...
            if (stackInfo.isPreview) {
                ...
                if (te != null) {
                    PlacementInfo placementInfo = new PlacementInfo(stack, player.getYawHead(), hit.subtract(0.0, hit.y, 0.0));
