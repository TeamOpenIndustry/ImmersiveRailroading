	...
	public void onUpdate() {
		...
		if (world.isRemote) {
			if (ConfigSound.soundEnabled) {
				if (this.horn == null) {
					this.horn = ImmersiveRailroading.proxy.newSound(this.getDefinition().horn, false, 100, gauge).shouldScalePitch(this.getDefinition().shouldScalePitch());
					this.idle = ImmersiveRailroading.proxy.newSound(this.getDefinition().idle, true, 80, gauge).shouldScalePitch(this.getDefinition().shouldScalePitch());
				}
